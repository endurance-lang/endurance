...
void showSymbolList(List* list,FILE* f) {
    ...
    while (aux != NULL) {
        if(aux->sym->symbolType == SymbolTypeVariable) {
            fprintf(f,"| \033[38;5;49m%-15s\033[0m | \033[38;5;208m%-15s\033[0m | \033[38;5;193m%-19d\033[0m |\n", aux->sym->id, aux->sym->data.variable.type, aux->sym->data.variable.address);
        } else if(aux->sym->symbolType == SymbolTypeType) {
            fprintf(f,"| \033[38;5;49m%-15s\033[0m | \033[38;5;208m%-15s\033[0m | \033[38;5;193m%-19s\033[0m |\n", aux->sym->id, "DATATYPE", "N/A");
        } else {
            fprintf(f,"| \033[38;5;49m%-15s\033[0m | \033[38;5;208m%-15s\033[0m | \033[38;5;193m%-19s\033[0m |\n", aux->sym->id, "FUNCTION", "N/A");
        }
        aux = aux->prox;
    }
